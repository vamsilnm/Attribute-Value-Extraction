...
def labeled_data_genaration(tagged_a,tagged_v,tagged_i,tagged_s,tagged_n,tagged_v_i,tagged_v_s):
	file_open_attributes = open('attributes_tagged_mobiles_1.json','r')
	...
	file_open_values = open('values_tagged_mobiles.json','r')
...
def extracting_fetures():
	...
	file_open = open('descriptions_mobiles.json','r')
	feature_set = []
...
def naive_bayes_model_training(X_train, X_test, Y_train, Y_test,X_test_text):
	...
	file_output = open('attributes_value_set_naivbayes_mobiles.csv','w')
...
if __name__ == '__main__':
	...
	features = extracting_fetures()
	X_train, X_test, Y_train, Y_test = train_data_test_data(features,tagged_a,tagged_v,tagged_i,tagged_s,tagged_n,tagged_v_i,tagged_v_s,0.30)
	X_train_numbers = []
	X_test_numbers = []
	for each_feature in X_train:
		each_feature_number = []
	...
	naive_bayes_model_training(np.array(X_train_numbers), np.array(X_test_numbers), np.array(Y_train), np.array(Y_test),X_test)

	# pos_tag_X_train = []
	# pos_tag_X_test = []
	# for each_element in X_train:
	# 	pos_tag_X_test.append(nltk.pos_tag each_element[0])
